...
(function firestoreAccessSuite() {
    ...
    async function runSellerProductRuleTest(options = {}) {
        const firebaseConfig = window.firebaseConfig || window.__FIREBASE_DEFAULT_CONFIG__;
        if (!firebaseConfig || !firebaseConfig.apiKey) {
            throw new Error('Missing firebaseConfig. Ensure firebase-config.js is loaded on this page.');
        }
        const email = (options.email || '').trim();
        const password = options.password;
        if (!email || !password) {
            throw new Error('Provide the seller email and password via runSellerProductRuleTest({ email, password }).');
        }
        const productData = typeof options.productData === 'object' && options.productData !== null ? options.productData : {};
        const appName = 'SellerRuleTester';
        let sellerApp = firebase.apps.find((app) => app.name === appName);
        if (!sellerApp) {
            sellerApp = firebase.initializeApp(firebaseConfig, appName);
        }
        const auth = sellerApp.auth();
        const db = sellerApp.firestore();
        try {
            await auth.signOut().catch(() => {});
            const credential = await auth.signInWithEmailAndPassword(email, password);
            const seller = credential.user;
            if (!seller) {
                throw new Error('Seller sign-in failed.');
            }
            const docId = `ruleTest_${seller.uid}_${Date.now()}`;
            const payload = Object.assign({
                name: 'Rule Test Product',
                description: 'Temporary record generated by runSellerProductRuleTest',
                price: 1,
                stock: 1,
                sellerId: seller.uid,
                ...
                ruleTest: true
            }, productData, { sellerId: seller.uid });
            await db.collection('products').doc(docId).set(payload);
            await db.collection('products').doc(docId).delete().catch(() => {});
            console.log('%cSeller product write succeeded (rules allowed).', 'color:#16A34A;font-weight:bold;');
            return { status: 'allowed', docId };
        } catch (error) {
            const code = error.code || 'unknown';
            console.error(`Seller product write blocked [${code}]`, error);
            return { status: 'blocked', code, message: error.message };
        } finally {
            try {
                await sellerApp.auth().signOut();
            } catch (signOutError) {
                console.warn('Failed to sign out test seller session', signOutError);
            }
        }
    }

    window.runSellerProductRuleTest = runSellerProductRuleTest;
    console.log('Run %crunFirestoreAccessTests()%c to execute Firestore access tests.', 'color:#DC2626;font-weight:bold;', 'color:inherit;');
    console.log('Use %crunSellerProductRuleTest({ email, password })%c to probe seller write permissions.', 'color:#2563EB;font-weight:bold;', 'color:inherit;');
})();
